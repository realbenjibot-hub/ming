...
Caden asked for options day trading and reaffirmed it after the risk was laid out. Claude logged it as an operator override. Scope: long calls and long puts only, day book only, paper. `day.instrument: options` in config.yaml; `options:` block holds the picker and exit rules. The engine picks the contract (expiry closest to 7 days inside 3 to 14, strike closest to delta 0.45, open interest at least 100, spread at most 10 percent, premium at least 0.30); Sol names the stock and the direction (up or down). Size is 4 percent of the cap in premium (was 10 on the first day: one 35 percent stop equalled the whole 3.5 percent daily cap and halted him after the first loser). A daily-cap halt now clears itself the next morning; a total-cap halt and the kill switch still wait for resume. Stops on options live in the engine and are checked by the scan every 2 minutes (Alpaca has no stop orders on contracts). Stock exits still use broker stops. `day.instrument: stock` puts it back. The two-week trial restarts on options from 09-23.

## 09-24 evening: guardrails on, rework parked to Friday
Caden's call: keep him running and unbroken through Friday, then a heavy rework. Shipped as pure guardrails: a day thesis older than 90 minutes is skipped as stale (morning theses were being bought at 13:48 after a halt cleared), macro ETFs (list in config) are skipped unless operator-directed, at most three day-book entries a day. Built but OFF until the rework, in config: tape confirmation at entry (relvol, VWAP side, extension in ATR; `day.confirm.enabled`) and the stock-based working stop for contracts (`options.underlying_stop`). Both depend on live bar data not yet exercised. Rework candidates for Friday: turn those on and verify against live data; stop on the underlying instead of the premium; entries on a signal rather than a timer; catalyst-type tagging so the weekly review can say what pays; revisit whether options or shares is the right instrument for a news edge. Two options days so far: 09-23 three stops (sizing was 10 percent, now 4), 09-24 three entries from stale morning theses after the halt cleared.
