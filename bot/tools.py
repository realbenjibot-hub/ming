...
TOOLS = [
  ...
  {"name": "run_command", "description": "Run one of: research, refresh, execute, review, report, reflect, hunt, scan, flatten, kill, resume. hunt is an intraday research pass plus day-book entries; scan checks the day book's stops and targets; flatten sells the day book; flatten_all sells both books. kill, resume, execute, hunt and flatten require confirmed=true, and you must first tell the operator exactly what will happen and get a yes.", "parameters": {"type": "object", "properties": {"command": {"type": "string"}, "confirmed": {"type": "boolean"}}, "required": ["command"]}},
  {"name": "set_setting", "description": "Change one risk setting by key (see get_state config), set aggression 1 to 10 with key 'aggression', or set hold_mode to 'day', 'swing', or 'both'.", "parameters": {"type": "object", "properties": {"key": {"type": "string"}, "value": {"type": ["number", "string"]}}, "required": ["key", "value"]}},
  ...
  {"name": "get_lessons", "description": "Read the lessons you wrote from your own closed trades, newest last.", "parameters": {"type": "object", "properties": {}}},
  {"name": "propose_trade", "description": "Operator asked to buy or sell a specific name. Returns what the rules engine would do (size, stop, target, which book) and what research thinks. Show this to the operator and ask for a yes before calling execute_trade. book is day or swing; omit to let the engine pick.", "parameters": {"type": "object", "properties": {"symbol": {"type": "string"}, "side": {"type": "string", "enum": ["buy", "sell"]}, "book": {"type": "string", "enum": ["day", "swing"]}}, "required": ["symbol"]}},
  {"name": "execute_trade", "description": "Place the trade the operator confirmed after propose_trade, in the same book. Requires confirmed=true.", "parameters": {"type": "object", "properties": {"symbol": {"type": "string"}, "side": {"type": "string", "enum": ["buy", "sell"]}, "book": {"type": "string", "enum": ["day", "swing"]}, "confirmed": {"type": "boolean"}}, "required": ["symbol", "confirmed"]}},
...
DESTRUCTIVE = {"kill", "resume", "execute", "hunt", "flatten", "flatten_all"}
...
def dispatch(svc, name, args):
    ...
    try:
        ...
        if name == "get_lessons": return {"lessons": svc.lessons() or "none yet"}
        if name == "propose_trade": return svc.exe.propose(args.get("symbol", ""), args.get("side", "buy"), args.get("book"))
        if name == "execute_trade":
            if not args.get("confirmed"): return {"ok": False, "needs_confirmation": True}
            return svc.exe.operator_trade(args.get("symbol", ""), args.get("side", "buy"), args.get("book"))
...
def _what(c):
    return {"kill": "cancel every open order, sell every position at market, and halt until the operator resumes",
            ...
            "flatten": "cancel every stop and sell every open day-book position at market",
            "flatten_all": "cancel every stop and sell every open position in both books at market"}.get(c, c)
def _trim(x):
