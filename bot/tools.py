...
TOOLS = [
  ...
  {"name": "run_command", "description": "Run one of: research, refresh, execute, review, report, kill, resume. kill, resume and execute require confirmed=true, and you must first tell the operator exactly what will happen and get a yes.", "parameters": {"type": "object", "properties": {"command": {"type": "string"}, "confirmed": {"type": "boolean"}}, "required": ["command"]}},
  {"name": "set_setting", "description": "Change one risk setting by key (see get_state config) or set aggression 1 to 10 with key 'aggression'.", "parameters": {"type": "object", "properties": {"key": {"type": "string"}, "value": {"type": "number"}}, "required": ["key", "value"]}},
...
DESTRUCTIVE = {"kill", "resume", "execute"}
...
def _what(c):
    return {"kill": "cancel every open order, sell every position at market, and halt until the operator resumes",
            ...
            "execute": "take today's theses through the rules engine and place real orders"}.get(c, c)
def _trim(x):
